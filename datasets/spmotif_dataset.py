...
class SPMotif(InMemoryDataset):
    ...
    def process(self):
        ...
        edge_index_list, label_list, correlation_list, ground_truth_list, role_id_list, pos = np.load(osp.join(self.raw_dir, self.raw_file_names[idx]), allow_pickle=True)
        data_list = []
        for idx, (edge_index, y, corr, ground_truth, z, p) in enumerate(zip(edge_index_list, label_list, correlation_list, ground_truth_list, role_id_list, pos)):
            ...
            corr = torch.tensor(corr, dtype=torch.long).unsqueeze(dim=0)
