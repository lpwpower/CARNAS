...
def split_batch(g): # Fixed: Last graph split missing when last graph (s) have no edges
    split = degree(g.batch[g.edge_index[0]], dtype=torch.long).tolist()
    while len(split) != g.batch[-1]+1:
        split.append(0)
    ...
    # if num_nodes.shape[0] != num_edges.shape[0]:
    # print('split',split)
    # print('split',len(split))
    # print('edge_indices',len(edge_indices))
    # print(num_nodes.shape, cum_nodes.shape, num_edges.shape, cum_edges.shape)  # edge_indices

    return edge_indices, num_nodes, cum_nodes, num_edges, cum_edges

        
def inv_split_graph(data, edge_score, ratio):
    ...
    edge_indices, num_nodes, cum_nodes, num_edges, cum_edges = split_batch(data)
    mean_num_edge = torch.mean(num_edges.to(dtype=torch.float))
    for edge_index, N, C in zip(edge_indices, num_edges, cum_edges):
        if N < int(ratio * mean_num_edge):
            n_reserve = N.cpu().numpy()
        else:
        ...
        rank = np.argpartition(-single_mask_detach, n_reserve-1)
    ...
    return (causal_edge_index, causal_edge_attr, causal_edge_weight), \
        (conf_edge_index, conf_edge_attr, conf_edge_weight), num_nodes, cum_nodes, num_edges
...
def causal_relabel(task, x, edge_index, batch, num_nodes, cum_nodes, num_edges, pos=None):
    all_num_nodes = x.size(0)
    # print('num_nodes',all_num_nodes,cum_nodes[-1]+num_nodes[-1])
    # print('edge_index',edge_index)
    sub_nodes = torch.unique(edge_index)
    # print('num_nodes',num_nodes)
    if task =='causal':
        no_edge_graph = (num_edges == 0).nonzero() # find graphs whose num of edge == 0
        # print('no_edge_graph',no_edge_graph)
        # print(num_nodes)
        # print(cum_nodes)
        for idx in no_edge_graph:
            # print(cum_nodes[idx[0]],num_nodes[idx[0]])
            idx_nodes = torch.arange(cum_nodes[idx[0]],cum_nodes[idx[0]]+num_nodes[idx[0]])
            # print('idx_nodes',idx_nodes)
            sub_nodes = torch.cat([sub_nodes, idx_nodes.to(device=sub_nodes.device)]) 
        sub_nodes = torch.unique(sub_nodes)
    # print("edge_index",edge_index)
    x = x[sub_nodes]
    # print(task,'batch before',batch)
    afterbatch = batch[sub_nodes]
    # print(task,'batch after',afterbatch)
    # if task =='causal' and batch[-1]!=afterbatch[-1]:
    #     print(task,'batch before',batch)
    #     print(task,'batch after',afterbatch)
    #     print('no_edge_graph',no_edge_graph)
    #     print(num_edges.shape) #511!=512
    #     for idx in no_edge_graph:
    #         idx_nodes = torch.arange(cum_nodes[idx[0]],cum_nodes[idx[0]]+num_nodes[idx[0]]+1)
    #         print(idx,idx_nodes.shape,num_nodes[idx[0]])
    #         sub_nodes = torch.cat([sub_nodes, idx_nodes.to(device=sub_nodes.device)]) 


    row, col = edge_index
    # remapping the nodes in the explanatory subgraph to new ids.
    node_idx = row.new_full((all_num_nodes,), -1)
    node_idx[sub_nodes] = torch.arange(sub_nodes.size(0), device=row.device)
    edge_index = node_idx[edge_index]
    if pos is not None:
        pos = pos[sub_nodes]
    return x, edge_index, afterbatch, pos
