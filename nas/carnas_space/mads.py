...
class Network(nn.Module):
  ...
  def forward(self, data, mode='none', graph_alpha = None):
    ...
    if isinstance(data, Data): #lpw
        x, edge_index = data.x, data.edge_index
        edge_attr = getattr(data, 'edge_attr', None).to(dtype=torch.long)
        batch = data.batch
    else: # causal_g/conf_g = (causal_x, causal_edge_index, causal_edge_attr, causal_edge_weight, causal_batch)
    ...
    if self.virtual:
        # print('batch:',batch)#,'batch[-1].item():',batch[-1].item())
    ...
    for i in range(self.num_layers):
        ...
        if mode == "mads":
            x = self.gnn_layers[i](x, na_alphas[i], edge_index, edge_weights, edge_attr.to(dtype=torch.long), with_linear, graph_match = True)
        else:
